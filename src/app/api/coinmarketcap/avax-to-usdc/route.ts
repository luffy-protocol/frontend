...
export async function GET(req: Request, res: Response) {
  try {
    ...
    const avaxToUsd = await getExchangeRate(COINMARKETCAP_IDS.avax);

    const usdcInUsd = usdcAmount * usdcToUsd;
    const avaxAmount = usdcInUsd / avaxToUsd;
    console.log(avaxAmount / 1000000000000000000);
    return Response.json({
      success: true,
      amount: avaxAmount / 1000000000000000000,
    });
  } catch (error) {
