...
export async function GET(req: Request, res: Response) {
  try {
    ...
    const ethToUsd = await getExchangeRate(COINMARKETCAP_IDS.eth);

    const usdcInUsd = usdcAmount * usdcToUsd;
    const ethAmount = usdcInUsd / ethToUsd;
    console.log(ethAmount / 1000000000000000000);
    return Response.json({
      success: true,
      amount: ethAmount / 1000000000000000000,
    });
  } catch (error) {
